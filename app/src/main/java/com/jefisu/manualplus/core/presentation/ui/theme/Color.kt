package com.jefisu.manualplus.core.presentation.ui.theme

import androidx.compose.ui.graphics.Color

val light_Primary = Color(0xFF663399)
val light_Secondary = Color(0xFF0061A2)
val light_Error = Color(0xFFba1a1a)

val dark_Primary = Color(0xFFd4bbff)
val dark_onPrimary = Color(0xFF41008b)
val dark_Secondary = Color(0xFF9DCAFF)
val dark_onSecondary = Color(0xFF003257)
val dark_Error = Color(0xFFffb4ab)
val dark_onError = Color(0xFF690005)

val Background = Color(0xFFE6E1E6)
val on_background = Color(0xFF252033)

val light_placerholderColor = on_background.copy(0.6f)
val dark_placerholderColor = Color.White.copy(0.6f)